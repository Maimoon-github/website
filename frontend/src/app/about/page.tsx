    ...
    <main className="min-h-screen bg-[#131026] py-32 text-[#E5DEFE]">
      <div className="max-w-[1440px] mx-auto px-10 relative z-10">
        <h1 className="text-5xl md:text-7xl font-black mb-12 font-display">
          {section?.title?.split(' ')[0] || "About"}{" "}
          <span className="text-[#8B65BF] font-light">
            {section?.title?.split(' ').slice(1).join(' ') || "Us"}
          </span>
        </h1>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-32">
          <div>
            <p className="text-xl leading-relaxed text-[#968E9C] font-medium mb-6 whitespace-pre-wrap">
            ...
            <div className="flex flex-col gap-8 mt-12">
              {section?.mission_statement && (
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-[#E5DEFE]">Our Mission</h3>
                    <p className="text-[#968E9C]">{section.mission_statement}</p>
                  </div>
                </div>
              )}
              ...
              {section?.vision_statement && (
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-[#E5DEFE]">Our Vision</h3>
                    <p className="text-[#968E9C]">{section.vision_statement}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="relative">
            ...
            <div className="relative border-4 border-[#1F1A40] rounded-2xl overflow-hidden aspect-square flex items-center justify-center bg-[#131026]">
              {section?.featured_image ? (
                ...
                <span className="text-[#8B65BF]/40 font-mono text-sm tracking-widest uppercase text-center px-8">
        ...
        {team.length > 0 && (
          <div className="mt-32">
            <h2 className="text-4xl font-black mb-16 text-center italic">
              ENGINEERING <span className="text-[#8B65BF]">NODE</span> DIRECTORY
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {team.map((member) => (
                <div key={member.id} className="bg-[#1F1A40]/30 border border-[#1F1A40] rounded-2xl p-8 hover:border-[#8B65BF]/40 transition-all group">
                  <div className="w-24 h-24 rounded-2xl overflow-hidden mb-6 border-2 border-[#8B65BF]/20 group-hover:border-[#8B65BF]/60 transition-all">
                    {member.photo ? (
                      <img 
                      ...
                      <div className="w-full h-full bg-[#131026] flex items-center justify-center">
                        <span className="text-[#8B65BF]/20 text-4xl">?</span>
                      </div>
                    )}
                  </div>
                  <h3 className="text-2xl font-bold text-[#E5DEFE] mb-1">{member.name}</h3>
                  <p className="text-[#8B65BF] font-mono text-xs tracking-tighter uppercase mb-4">{member.role}</p>
                  <p className="text-[#968E9C] text-sm leading-relaxed mb-6">{member.bio}</p>
                  <div className="flex gap-4">
                    {member.linkedin_url && (
                      <a href={member.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-[#968E9C] hover:text-[#8B65BF] transition-colors">
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.761 0 5-2.239 5-5v-14c0-2.761-2.239-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                      </a>
                    )}
                    {member.twitter_url && (
                      <a href={member.twitter_url} target="_blank" rel="noopener noreferrer" className="text-[#968E9C] hover:text-[#8B65BF] transition-colors">
